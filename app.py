...
def add_task():
    ...
    # Store duration in description field
    duration = data.get('duration', 30)
    
    cursor.execute(
        'INSERT INTO tasks (id, name, color, created_at) VALUES (?, ?, ?, ?)',
        (task_id, data.get('name'), data.get('color'), datetime.now().isoformat())
    )
    
    # Store duration info in a settings-like way
    cursor.execute(
        'INSERT OR REPLACE INTO task_durations (task_id, duration, start_date, end_date) VALUES (?, ?, ?, ?)',
        (task_id, duration, data.get('start_date'), data.get('end_date'))
    )
...
def delete_task(task_id):
    ...
    cursor.execute('DELETE FROM task_durations WHERE task_id = ?', (task_id,))
    conn.commit()
    conn.close()
...
@app.route('/api/task-durations', methods=['GET'])
def get_task_durations():
    """Fetch all task durations"""
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute('SELECT task_id, duration, start_date, end_date FROM task_durations')
    rows = cursor.fetchall()
    conn.close()
...
def get_stats(task_id):
    ...
    # ✅ FIX #1: Get duration from TASK_DURATIONS table, not global settings
    cursor.execute('SELECT duration FROM task_durations WHERE task_id = ?', (task_id,))
...
def get_progress_data(task_id):
    ...
    # ✅ FIX #2: Get duration from TASK_DURATIONS table, not global settings
    cursor.execute('SELECT duration FROM task_durations WHERE task_id = ?', (task_id,))
    duration_row = cursor.fetchone()
    duration = duration_row['duration'] if duration_row else 30
...
def get_overall_progress():
    ...
    # ✅ FIX #3: Get max duration from task_durations, not global settings
    cursor.execute('SELECT MAX(duration) as max_dur FROM task_durations')
    max_dur_row = cursor.fetchone()
    duration = max_dur_row['max_dur'] if max_dur_row and max_dur_row['max_dur'] else 30
